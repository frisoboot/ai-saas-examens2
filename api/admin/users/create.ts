 ...
 *
 * Maakt een nieuwe gebruiker aan via Supabase Admin API.
...
const parseBody = (req: VercelRequest): { email?: string; password?: string } | null => {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (createError || !createdUser?.user) {
      ...
      return res.status(500).json({ error: createError?.message || 'Kon gebruiker niet aanmaken' });
    }
