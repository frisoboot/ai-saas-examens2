...
import { ExamPdfManager } from './ExamPdfManager';
...
export const ExamLibrary: React.FC<ExamLibraryProps> = ({ onEditQuestion }) => {
  ...
  const [managingPdfs, setManagingPdfs] = useState<string | null>(null); // key: "subject-year-level"
                        ...
                        <div key={yearKey}>
                          ...
                          {isYearExpanded && (
                            <div className="bg-white border-t border-slate-100">
                              ...
                              {yearGroup.year > 0 && (
                                <div className="flex flex-wrap items-center gap-2 px-16 py-2 bg-indigo-50/50 border-b border-slate-100">
                                  {(['VMBO-TL', 'HAVO', 'VWO'] as StudentLevel[]).map(lvl => {
                                    ...
                                    const hasKaartPdf = levelQuestions.some(q => q.examKaartUrl);
                                    if (!hasOpdrachtPdf && !hasBijlagePdf && !hasKaartPdf) return null;
                                    return (
                              ...
                              {/* Exam actions per level: PDF management + delete */}
                              {yearGroup.year > 0 && (
                                <div className="flex flex-wrap items-center gap-2 px-16 py-2 bg-slate-50 border-b border-slate-100">
                                  <span className="text-xs text-slate-500 mr-1">Acties:</span>
                                  {(['VMBO-TL', 'HAVO', 'VWO'] as StudentLevel[]).map(lvl => {
                                    const count = yearGroup.byLevel[lvl].length;
                                    if (count === 0) return null;
                                    const examKey = `${subjectGroup.subject}-${yearGroup.year}-${lvl}`;
                                    const isDeleting = deletingExam === examKey;
                                    const isManagingPdf = managingPdfs === examKey;
                                    return (
                                      <div key={lvl} className="flex items-center gap-1">
                                        <button
                                          onClick={() => setManagingPdfs(isManagingPdf ? null : examKey)}
                                          className={`flex items-center gap-1 text-xs px-2 py-1 rounded border transition-colors ${
                                            isManagingPdf
                                              ? 'bg-indigo-100 text-indigo-700 border-indigo-300'
                                              : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50'
                                          }`}
                                          title={`PDF's beheren voor ${subjectGroup.subject} ${yearGroup.year} ${lvl}`}
                                        >
                              ...
                              {/* PDF Manager panel (shown when managing PDFs for a specific level) */}
                              {yearGroup.year > 0 && (['VMBO-TL', 'HAVO', 'VWO'] as StudentLevel[]).map(lvl => {
                                const examKey = `${subjectGroup.subject}-${yearGroup.year}-${lvl}`;
                                if (managingPdfs !== examKey) return null;
                                const levelQuestions = yearGroup.byLevel[lvl];
                                if (levelQuestions.length === 0) return null;
                                return (
                                  <div key={`pdf-${examKey}`} className="px-16 py-3 border-b border-slate-100">
                                    <ExamPdfManager
                                      subject={subjectGroup.subject}
                                      year={yearGroup.year}
                                      level={lvl}
                                      questions={levelQuestions}
                                      onClose={() => setManagingPdfs(null)}
