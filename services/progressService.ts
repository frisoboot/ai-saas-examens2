...
export const getStudentProgress = async (studentName: string): Promise<StudentProgress[]> => {
  ...
  // LocalStorage fallback - calculate from exam results
  const resultsKey = 'ai_exam_results';
  const stored = localStorage.getItem(resultsKey);
  const allResults: ExamResult[] = stored ? JSON.parse(stored) : [];

  const studentResults = allResults.filter(r => r.studentName === studentName);
  const subjects = [...new Set(studentResults.map(r => r.subject))];

  return subjects.map(subject => calculateProgress(studentName, subject, allResults));
};
...
export const updateProgressAfterExam = async (result: ExamResult): Promise<void> => {
  if (!supabase) {
    // For localStorage mode, progress is calculated on-the-fly
    return;
  }
